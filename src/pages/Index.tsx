  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center mb-6">
...
const Index = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // 1. تحريك الـ Hero Section عند التحميل
      const tl = gsap.timeline({ defaults: { ease: "expo.out" } });
      
      tl.from(".hero-badge", { y: 30, opacity: 0, duration: 1 })
        .from(".hero-title", { y: 50, opacity: 0, duration: 1.2 }, "-=0.8")
        .from(".hero-desc", { y: 30, opacity: 0, duration: 1 }, "-=1")
        .from(".hero-btns", { y: 20, opacity: 0, duration: 0.8 }, "-=0.8")
        .from(".hero-visual", { scale: 0.9, opacity: 0, duration: 1.5 }, "-=1");

      // 2. تحريك الـ Floating Orbs بشكل مستمر
      gsap.to(".orb-1", { x: 50, y: -50, duration: 10, repeat: -1, yoyo: true, ease: "sine.inOut" });
      gsap.to(".orb-2", { x: -60, y: 40, duration: 12, repeat: -1, yoyo: true, ease: "sine.inOut" });

      // 3. تحريك البطاقات (Features) عند التمرير
      ...
      // 4. تأثير الـ Parallax على الـ Hero Visual
      gsap.to(".hero-visual", {
        scrollTrigger: {
    ...
    return () => ctx.revert(); // تنظيف الذاكرة عند إغلاق الصفحة
  }, []);
    ...
    <div ref={mainRef} className="min-h-screen bg-background overflow-hidden selection:bg-primary/30">
      ...
      <section className="hero-section relative min-h-screen flex items-center justify-center py-20">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="orb-1 absolute top-1/4 left-1/4 w-96 h-96 bg-primary/20 rounded-full blur-[120px]" />
          <div className="orb-2 absolute bottom-1/4 right-1/4 w-80 h-80 bg-accent/20 rounded-full blur-[100px]" />
        </div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center max-w-5xl mx-auto">
            ...
            <h1 ref={heroTextRef} className="hero-title text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold text-foreground mb-8 leading-[1.1] tracking-tight">
              Build AI Agents
              <span className="block text-gradient-primary mt-2">That Think Together</span>
            </h1>

            <p className="hero-desc text-lg md:text-xl lg:text-2xl text-muted-foreground max-w-3xl mx-auto mb-12 leading-relaxed">
            ...
            <div className="hero-btns flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
              <Link to="/auth">
                <Button size="lg" className="gap-3 px-10 py-7 text-lg rounded-2xl bg-primary hover:scale-105 transition-transform duration-300 shadow-xl shadow-primary/20">
          ...
          {/* Hero Visual */}
          <div className="hero-visual mt-20 relative mx-auto max-w-6xl rounded-3xl border border-border/50 bg-card/40 backdrop-blur-xl p-2 shadow-2xl">
              <div className="rounded-2xl bg-gradient-to-br from-card to-card/50 p-10 min-h-[350px] flex items-center justify-center relative overflow-hidden">
                <div className="grid grid-cols-3 gap-12 relative z-10">
                  {[ {I: Bot, t: "AI Agents", c: "violet"}, {I: Workflow, t: "Workflows", c: "cyan"}, {I: Database, t: "Knowledge", c: "emerald"} ].map((item, i) => (
                    <div key={i} className="flex flex-col items-center gap-4 hover:scale-110 transition-transform cursor-pointer">
